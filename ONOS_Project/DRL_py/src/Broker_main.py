...
is_domain1_blocked = True
is_domain2_blocked = True
...
'''
linkmap[2][19] = (1000, 600)
linkmap[19][2] = (1009, 600)

linkmap[2][28] = (1001, 1200)
linkmap[28][2] = (1010, 1200)

linkmap[6][23] = (1002, 1200)
linkmap[23][6] = (1011, 1200)

linkmap[8][19] = (1003, 1400)
linkmap[19][8] = (1012, 1400)

linkmap[8][23] = (1004, 2000)
linkmap[23][8] = (1013, 2000)

linkmap[8][24] = (1005, 1400)
linkmap[24][8] = (1014, 1400)

linkmap[11][19] = (1006, 800)
linkmap[19][11] = (1015, 800)

linkmap[11][23] = (1007, 600)
linkmap[23][11] = (1016, 600)

linkmap[12][28] = (1008, 800)
linkmap[28][12] = (1017, 800)
...
linkmap[19][15] = (2000, 1100)
linkmap[23][15] = (2001, 1200)
linkmap[24][15] = (2002, 1800)
linkmap[28][15] = (2003, 1400)
'''
...
LINK_NUM = 9 #TODO:should know this by info, only used when cal resource util

NODE_NUM = 28 #

model2_flag = 0  # number of future time slots to look into

N = 10 #10  # number of paths each src-dest pair
...
Candidate_Paths = []
'''
def matlab_cal_candidate_path():
    global input_Top
    global Candidate_Paths

    engine = matlab.engine.start_matlab()
    status = engine.generate_candidate_path(input_Top) #generate Src_Dst_Paths_Inter_Top.dat
    print(status)
    engine.exit()
    ...
    fp = open('Src_Dst_Paths_Inter_Top.dat', 'rb')
    for ii in range(1, NODE_NUM * NODE_NUM + 1):  # NODE_NUM*NODE_NUM import precalculated paths (in terms of path_links)
        #    temp_path = []
        if ii % NODE_NUM == 0:
            i = ii // NODE_NUM
...
'''
#node1, node2, link_len  node1<node2
input_Top.append([2, 19, 600])
input_Top.append([2, 28, 1200])
input_Top.append([6, 23, 1200])
input_Top.append([8, 19, 1400])
input_Top.append([8, 23, 2000])
input_Top.append([8, 24, 1400])
input_Top.append([11, 19, 800])
input_Top.append([11, 23, 600])
input_Top.append([12, 28, 800])

#temp test
input_Top.append([1, 2, 1000])
input_Top.append([1, 6, 2000])
input_Top.append([1, 8, 1200])
input_Top.append([1, 11, 1200])
input_Top.append([1, 12, 800])

input_Top.append([15, 19, 1100])
input_Top.append([15, 23, 1200])
input_Top.append([15, 24, 1800])
input_Top.append([15, 28, 1400])
'''
    ...
    def get_new_slot_temp(self, slot_temp, path_link, slot_map):
        ...
        for i in path_link:
            for j in range(self.SLOT_TOTAL):
                slot_temp[j] = slot_map[link_id2idx[str(i)]][j] & slot_temp[j]
        return slot_temp
            ...
            while not coord.should_stop():
                ...
                while episode_step_count < self.episode_size:
                    ...
                    if len(path) == 0:  # selected an invalid action
                        blocking = 1
                        result_queue.put([-2, [], -2, -2, -2, current_src, current_dst])
                    else:
                        path_len = self.cal_len(path)  # physical length of the path
                        num_FS = self.cal_FS(current_bandwidth, path_len)
                        slot_temp = [1] * self.SLOT_TOTAL
                        ...
                        if flag == 1:
                            ...
                            result = []
                            '''
                            result = [req_id, list(path_links), fs_start, fs_end, current_TTL, current_src, current_dst]
                            result_queue.put(result)

                        else:
                            blocking = 1
                            result_queue.put([-1, list(path_links), -1, -1, -1, current_src, current_dst])
                    ...
                    resource_util.append(1 - np.sum(self.slot_map) / (self.LINK_NUM * self.SLOT_TOTAL))# not correct for broker

                    if episode_count < (3000 / self.episode_size):  # for warm-up once
                        continue
                ...
                if self.name == 'agent_0':
                    ...
                    fp = open("Broker_Episode_Time2.log", "a")
...
class DRLThread(threading.Thread):
    ...
    def pack_sendmsg(self):
        global result_queue
        global is_domain1_blocked
        global is_domain2_blocked
        #result_queue.put([-2, [], -2, -2, 2, current_src, current_dst])
        send_list = []
        send_map = {}
        if is_domain1_blocked or is_domain2_blocked:
            send_map["req_id"] = -2
            send_map["path_links"] = []
    ...
    def handle_recvmsg_linkmap(self, jresponse):
        ...
        for key in jresponse.keys():
            ...
            if link_id < 1000:
                is_domain1_blocked = False
            elif link_id >= 2000:
                is_domain2_blocked = False
        #matlab_cal_candidate_path()
        return
    ...
    def run(self):
        ...
        ip_port = (read_Cfg.Broker_IPv4, int(read_Cfg.Broker_Port_DRL))
        ...
        while True:
            ...
            if lock.acquire():
                if recv_status == EXPECT_REQ:
                    ...
                    if is_domain2_blocked or is_domain1_blocked:
                        is_result_ready = True
                    else:
