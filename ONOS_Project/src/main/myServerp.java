...
*/
    ...
    private static int CTRL2_NOT_READY = -1;
    ...
    private static int current_status = CTRL2_NOT_READY;
    ...
    private static List<myServerp.DRLThread.DRLrecv_req> DRLrecvreqs = null;
    ...
    private class DRLThread implements Runnable{
        ...
        private void send_map() throws IOException{
            ...
            //System.out.println("CTRL2->DRL: " + send_json_string);
            //String logstr = "CTRL2->DRL: " + send_json_string + "\n";
        ...
        private void parse_recv() throws IOException{
            ...
            //System.out.println("DRL->CTRL2: " + recv_str);
            //String logstr = "DRL->CTRL2: " + recv_str + "\n";
            //loger.write(logstr.getBytes());
        }
    ...
    private class BrokerThread implements Runnable {
        ...
        private void send_list() throws IOException{
            ...
            this.send_json_stream.write(send_json_byte);
            this.send_json_stream.flush();
            //System.out.println("CTRL2->Broker: " + send_json_string);
            //String logstr = "CTRL2->Broker: " + send_json_string + "\n";
            //loger.write(logstr.getBytes());
        }

        private void parse_recv() throws IOException{
            JSONObject recvjsonObj = JSONObject.fromObject(recv_str);
            req_id = recvjsonObj.getInt("req_id");

            //do not need src here
            current_dst = recvjsonObj.getInt("dst");
            current_bandwidth = recvjsonObj.getInt("bandwidth");
            current_TTL = recvjsonObj.getDouble("TTL");
            //System.out.println("Broker->CTRL2: " + recv_str);
            //String logstr = "Broker->CTRL2: " + recv_str + "\n";
            //loger.write(logstr.getBytes());
        }
            ...
            try{
                ...
                while(run_flag) {
                    if(is_Broker_Ready == false){
                        Map<String, String> firstSend = new HashMap<>();
                        firstSend.put("Status", "OK");
                        this.send_map_data = firstSend;
                        send_map();
                    ...
                    if(is_reward == false) {
                        ...
                        LockSupport.unpark(mainThread);
                        LockSupport.park();
                        //will be unparked by mainthread when result is ready
                        send_list();
                        is_reward = true;
                    }
                    else {
    ...
    private class networkInfo{
        ...
        public int[] bnodes_2domain1 = {5, 9 ,10, 14};//{19,23,24,28}; //border nodes to domain2
        ...
        */
            ...
            if(is_inter) {
                for (int i = 0; i < bnodes_2domain1.length; i++) {
                    ...
                    if(bnodes_2domain1[i] != current_dst) {
                        send_valuemap.put(bnodename, String.valueOf(bnodes_2domain1[i]));
                    }
                }
            }
        ...
        private List<Map> pack_sendmsg2Broker(){
            ...
            //int new_link_id = 0;//check if new linkid work
            for(myServerp.DRLThread.DRLrecv_req DRLrecvreq:DRLrecvreqs){
                Map<String, String> send_valuemap = new HashMap<String, String>();
    ...
    public void main(String[] args) throws Exception{
        ...
        myServerp maintest = new myServerp();
        ...
        */
        ...
        //DRL port:20007
        ServerSocket Server_DRL = new ServerSocket(Integer.parseInt(readCfg.Serverp_Port_DRL));
        ...
        Socket Client_Broker = new Socket(readCfg.Broker_IPv4, Integer.parseInt(readCfg.Broker_Port_CTRL2));
        ...
        int total_num = 0;
        loger = new FileOutputStream("./log/CTRLer2.log");

        //main loop
        while(true) {
            if(current_status == CTRL2_NOT_READY) {
                ...
                loger.write(logstr.getBytes());
                if(total_num % 2000 == 0){
                    String writestring = "Requests_Total: " + String.valueOf(total_num) + "\n";
            ...
            else if(current_status == REQ0_DRL00_BROKER00_RWD00){
                //recv req from broker
                LockSupport.park();
                //will be unparked by Broker thread after recv inter-domain request
                ...
                LockSupport.park();
                //will be unparked by BrokerThread after recv finalreward
                current_status = REQ1_DRL11_BROKER11_RWD00;
            }
            else if(current_status == REQ1_DRL11_BROKER11_RWD00){
                //get reward, update slotmap, request set
                ...
                if(status == -2){
                    ...
                    System.out.println("reward status error");
                }
                ...
                current_status = CTRL2_NOT_READY;
            }
